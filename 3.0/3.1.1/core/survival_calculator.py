...
class SurvivalCalculator:
    ...
    def calculate_survival_data(self, df, per_group_names, big_group_nums, per_group_nums, sheet_name=""):
        ...
        # 计算分组统计（mean和SE）
        group_data, pltdata, pltdata_errorbars = self._calculate_survival_group_statistics(
    ...
    def _calculate_survival_group_statistics(self, df_sur, per_group_names, big_group_nums, per_group_nums):
        """计算分组统计"""
        group_data = []
    ...
    def _clean_plot_data(self, rawdata):
        """清理raw数据"""
        rawdata['testzero'] = rawdata.iloc[:, 1:].mean(axis=1)
        rawdata = rawdata[rawdata['testzero'] != 0]
        rawdata = rawdata.drop(columns=['testzero'])
        
        zerorow = [rawdata.iloc[-1, 0] + 2] + [0] * (rawdata.shape[1] - 1)
        zerorow = pd.DataFrame([zerorow], columns=rawdata.columns)
        rawdata = pd.concat([rawdata, zerorow], ignore_index=True)
