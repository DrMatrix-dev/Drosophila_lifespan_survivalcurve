...
class ExcelDataPreprocessor:
    ...
    def process_sheet_data(self, df, sheet_name):
        ...
        logrank_test_open = False
        if df.isin(['group_type']).any().any():
            logrank_test_open = True
        ...
        if(group_types_names is not None):
            per_group_type_nums, per_group_types = self._calculate_group_info(group_types_names)
            print("每组类型：", per_group_types)
            #breakpoint()
        #df = df.reset_index(drop=True)
        #df.columns = pd.RangeIndex(start=0, stop=len(df.columns), step=1)
        ...
        return df, gender, per_group_names, big_group_nums, per_group_nums, per_group_types, logrank_test_open
